...
int DirectoryListingFrame::ItemInfo::getImage(int col) const {
	...
	return static_cast<int>(WinUtil::getFileIcon(getText(COLUMN_FILENAME)));
}
	...
	searching(false)
{
...

	{
		...
		{
			ids.emplace_back(std::to_string(ids.size()));
	...
	~FileListLoader() {
		...
		for(auto& i: cache) {
			if(parent.fileCache.find(i.first) == parent.fileCache.end()) {
				...
				cacheCount -= static_cast<uint32_t>(i.second.size());
			}
		}
	}
...
void DirectoryListingFrame::postClosing() {
	...
	cacheCount -= static_cast<uint32_t>(dirCache.size());
	for(auto& i: fileCache) { cacheCount -= static_cast<uint32_t>(i.second.size()); }
...
void DirectoryListingFrame::handleFindToggle() {
	if(searchGrid->getEnabled()) {
		...
		for(auto it = lastSearches.rbegin(); it != lastSearches.rend(); ++it) {
			auto p = it->get();
...
void DirectoryListingFrame::addTargets(Menu* menu, ItemInfo* ii) {
	...
	size_t i = 0;
	for(; i < spl.size(); ++i) {
		sub->appendItem(Text::toT(spl[i].second), [this, i] { handleDownloadFavorite(static_cast<unsigned>(i)); });
	}
	...
	if(ii && ii->type == ItemInfo::FILE) {
		...
		if(!targets.empty()) {
			...
			for(i = 0; i < targets.size(); ++i) {
				sub->appendItem(Text::toT(targets[i]), [this, i] { handleDownloadTarget(static_cast<unsigned>(i)); });
			}
		}
	}
	...
	if(!WinUtil::lastDirs.empty()) {
		...
		for(i = 0; i < WinUtil::lastDirs.size(); ++i) {
			sub->appendItem(WinUtil::lastDirs[i], [this, i] { handleDownloadLastDir(static_cast<unsigned>(i)); });
		}
	}
}
...
void DirectoryListingFrame::updateStatus() {
	if(!searching && !updating) {
		int cnt = static_cast<int>(files->countSelected());
		int64_t total = 0;
		if(cnt == 0) {
			cnt = static_cast<int>(files->size());
...
void DirectoryListingFrame::handleSelectionChanged() {
	...
	pathBox->setSelected(static_cast<int>(historyIndex - 1));
...
void DirectoryListingFrame::changeDir(DirectoryListing::Directory* d) {
	...
	for(auto& i: d->directories) {
		files->insert(static_cast<int>(files->size()), getCachedDir(i));
	}
	...
	if(useCache) {
		...
		for(auto& i: cache->second) {
			files->insert(static_cast<int>(files->size()), &i);
		}
	} else {
		dcdebug("creating file info on-the-fly\n");
		for(auto& i: d->files) {
			files->insert(static_cast<int>(files->size()), new ItemInfo(i));
		}
	}
