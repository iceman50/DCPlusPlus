...
temporary(temporary)
{
	setIcon(WinUtil::fileImages->getIcon(static_cast<unsigned>(WinUtil::getFileIcon(path))));
...
void TextFrame::layout() {
	...
	r.size.y -= static_cast<long>(grid->getSpacing()); // add a bottom margin not to be too stuck to the status bar.
	grid->resize(r);
}
