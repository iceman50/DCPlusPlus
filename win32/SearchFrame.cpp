...
int SearchFrame::SearchInfo::getImage(int col) const {
	...
	return sr->getType() == SearchResult::TYPE_FILE ? static_cast<int>(WinUtil::getFileIcon(sr->getFile())) : static_cast<int>(WinUtil::DIR_ICON);
}
...
int SearchFrame::SearchInfo::compareItems(const SearchInfo* a, const SearchInfo* b, int col) {
	switch(col) {
	case COLUMN_HITS: return compare(static_cast<int64_t>(a->srs.size()), static_cast<int64_t>(b->srs.size()));
	case COLUMN_TYPE:
...
void SearchFrame::addToList(SearchInfo* si) {
	...
	if(filter.empty() || filter.match(filter.prepare(), [this, si](size_t column) { return Text::fromT(si->getText(static_cast<int>(column))); })) {
		if(i == -1) {
			if(si->parent) {
...
void SearchFrame::updateList() {
	...
	auto filterInfoF = [this, &i](size_t column) { return Text::fromT(i->getText(static_cast<int>(column))); };
...
void SearchFrame::onHubChanged(HubInfo* info) {
	int nItem = 0;
	auto n = static_cast<int>(hubs->size());
	for(; nItem < n; nItem++) {
...
void SearchFrame::runSearch() {
	...
	StringList clients;
	for(unsigned i = 0, n = static_cast<unsigned>(hubs->size()); i < n; ++i) {
		if(hubs->isChecked(i)) {
	...
	if(find(lastSearches.begin(), lastSearches.end(), s) == lastSearches.end())
	{
		...
		if(searchBox->size() > i)
			searchBox->erase(static_cast<int>(i));
...
void SearchFrame::updateStatusCount() {
	auto current = static_cast<uint64_t>(results->size());
	auto total = static_cast<uint64_t>(searchResults.size());
	if(current >= total) {
