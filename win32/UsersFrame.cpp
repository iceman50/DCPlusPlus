...
void UsersFrame::updateList() {
	...
	auto filterInfoF = [this, &i](size_t column) { return Text::fromT(i->second.getText(static_cast<int>(column))); };
...
bool UsersFrame::matches(const UserInfo &ui) {
	if(!filter.empty() && !filter.match(filter.prepare(), [this, &ui](size_t column) { return Text::fromT(ui.getText(static_cast<int>(column))); })) {
		return false;
	}
