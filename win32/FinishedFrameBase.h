...
private:
	...
	public:
		...
		int getImage(int col) const {
			return col == 0 ? static_cast<int>(WinUtil::getFileIcon(file)) : -1;
		}
	...
	void updateStatus() {
		...
		this->status->setText(STATUS_COUNT, str(TFN_("%1% item", "%1% items", static_cast<unsigned long>(count)) % count));
	...
	FileInfo* findFileInfo(const string& file) {
		for(int i = 0, n = static_cast<int>(files->size()); i < n; ++i) {
	...
	UserInfo* findUserInfo(const HintedUser& user) {
		for(int i = 0, n = static_cast<int>(users->size()); i < n; ++i) {
