...
fileLists(0)
{
...

	{
		...
		{
			toolbarIds.emplace_back(std::to_string(toolbarIds.size()));
...
void QueueFrame::addQueueItem(QueueItemPtr&& ii, bool single) {
	...
	if(!SETTING(QUEUEFRAME_SHOW_TREE) || isCurDir(dir)) {
		...
		if(single) {
			...
			files->insert(static_cast<int>(files->size()), p); // no sort (resort the whole list later)
		}
	}
}
...
void QueueFrame::updateFiles() {
	...
	for(auto j = i.first; j != i.second; ++j) {
		auto& ii = j->second;
		ii->update();
		files->insert(static_cast<int>(files->size()), ii.get());
	}
...
void QueueFrame::moveSelected() {
	int n = static_cast<int>(files->countSelected());
	if(n == 1) {
...
void QueueFrame::reEvaluateToolbar() {
	...
	if(usingDirMenu)
		cnt = static_cast<int>(dirs->countSelected());
	else
		cnt = static_cast<int>(files->countSelected());
	...
	if(toolbarItemsStatus != statusToolbar) {
		...
		for(auto& i : toolbarIds) {
			if(!i.empty()) {
				toolbar->setButtonEnabled(static_cast<unsigned>(std::stoul(i)), statusToolbar);
			}
		}
	}
}
