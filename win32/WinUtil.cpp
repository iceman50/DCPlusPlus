	...
	while((i = uc.getCommand().find("%[line:", i)) != string::npos) {
		...
		if(find(names.begin(), names.end(), name) == names.end()) {
			...
			if(combo_values.size() > 2) { // must contain at least: caption, default sel, 1 value
				...
				try { combo_sel = static_cast<int>(std::stoull(Text::fromT(*first))); }
				catch(const std::invalid_argument&) { combo_sel = -1; }
				catch(const std::out_of_range&) { combo_sel = -1; }
				combo_values.erase(first);
			...
			if(combo_sel >= 0) {
				...
				auto prev = find(combo_values.begin(), combo_values.end(), Text::toT(std::get<string>(params["line:" + name])));
				...
				dlg.addTextBox(caption, Text::toT(std::get<string>(params["line:" + name])));
			}
...
bool registerHandler_(const tstring& name, const tstring& descr, bool url, const tstring& prefix) {
	...
	::RegSetValueEx(hk, NULL, 0, REG_SZ, (LPBYTE) descr.c_str(), static_cast<DWORD>(sizeof(TCHAR) * (descr.size() + 1)));
	if(url) {
	...
	bool ret = ::RegSetValueEx(hk, _T(""), 0, REG_SZ, (LPBYTE) app.c_str(), static_cast<DWORD>(sizeof(TCHAR) * (app.length() + 1))) == ERROR_SUCCESS;
	::RegCloseKey(hk);
		...
		0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hk, NULL) == ERROR_SUCCESS)
	{
		...
		::RegSetValueEx(hk, _T(""), 0, REG_SZ, (LPBYTE) app.c_str(), static_cast<DWORD>(sizeof(TCHAR) * (app.length() + 1)));
		::RegCloseKey(hk);
	}
